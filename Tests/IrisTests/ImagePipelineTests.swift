...
struct ImagePipelineTests {
    ...
    @Test func normalizeURL_validJPEGFile_returnsData() throws {
        let jpegData = Self.minimalJPEGData()
        let url = URL(fileURLWithPath: "/tmp/iris_test_\(UUID().uuidString).jpg")
    ...
    @Test func normalizeData_supportedMIMETypes_succeed() {
        let supportedTypes = [
    ...
    @Test func normalizeData_unsupportedMIMEType_throwsImageUnreadable() throws {
        let data = Data([0x25, 0x50, 0x44, 0x46]) // %PDF
        #expect(throws: IrisError.self) {
            _ = try ImagePipeline.normalize(data: data, mimeType: "application/pdf")
        }
        do {
            _ = try ImagePipeline.normalize(data: data, mimeType: "application/pdf")
        } catch IrisError.imageUnreadable(let reason) {
    ...
    @Test func normalizeData_textPlainMIMEType_throwsImageUnreadable() throws {
        do {
            _ = try ImagePipeline.normalize(data: Data("hello".utf8), mimeType: "text/plain")
    ...
    /// Returns a minimal 1×1 white JPEG byte sequence.
    private static func minimalJPEGData() -> Data {
        // A valid 1×1 white JPEG (generated offline, stable bytes).
        let bytes: [UInt8] = [
