...
@Suite("IrisProvider.gemini", .serialized)
struct GeminiProviderTests {

    // MARK: - Mock Response Bodies

    private let geminiSuccessJSON = """
    {"candidates":[{"content":{"parts":[{"text":"{}"}]}}]}
    """.data(using: .utf8)!

    private let gemini403JSON = """
    ...
    @Test func http403WithApiKeyErrorThrowsInvalidAPIKey() async throws {
        ...
        let provider = IrisProvider.gemini(apiKey: "bad-key", model: "gemini-2.0-flash", session: session)
