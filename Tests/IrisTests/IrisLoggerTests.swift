...
@Suite("IrisLogger")
struct IrisLoggerTests {

    @Test("image logger exists")
    func imageLoggerExists() {
        let _ = IrisLogger.image
    }

    @Test("network logger exists")
    func networkLoggerExists() {
        let _ = IrisLogger.network
    }

    @Test("decode logger exists")
    func decodeLoggerExists() {
        let _ = IrisLogger.decode
    }
    ...
    func parseFailurePropagatesWithLoggingActive() async throws {
        ...
        } catch IrisError.networkError {
            // Expected — logging must not suppress the error
        }
    }
    ...
    func imageNormalizationFailurePropagatesWithLoggingActive() async {
        ...
        } catch IrisError.imageUnreadable {
            // Expected — image logger logged the failure, then rethrew
        } catch {
            Issue.record("Expected IrisError.imageUnreadable, got: \(error)")
        }
    }
}
...
// minimalJPEGData() is `private` in IrisClientTests.swift — redeclared here
