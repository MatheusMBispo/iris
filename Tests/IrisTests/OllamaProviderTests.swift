...
@Suite("IrisProvider.ollama", .serialized)
struct OllamaProviderTests {

    // MARK: - Mock Response Bodies

    private let ollamaSuccessJSON = """
    {"message":{"content":"{}"}}
    """.data(using: .utf8)!
    ...
    @Test func connectionRefusedThrowsNetworkError() async throws {
        ...
        if case .networkError = caught as? IrisError {
            // expected — typical when Ollama is not running
        } else {
    ...
    @Test func requestBodyContainsStreamFalseAndBase64Image() async throws {
        ...
        #expect(normalizedBody.contains("\"stream\":false"))
